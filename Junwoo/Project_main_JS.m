...
for i = 1:length(width_1)
    LFP = zeros(1:length(dataInterest);
    LFP_temp{i,1} = peak_trial(i)*normpdf(peakTime_trial(i)-width_1(i)/0.32:peakTime_trial(i)+width_1(i)/0.32,peakTime_trial(i),width_1(i));
    LFP_norm{i,1} = LFP{i,1}/max(LFP{i,1});
    
    plot((peakTime_trial(i)-width_1(i)/0.32:peakTime_trial(i)+width_1(i)/0.32),LFP_norm{i,:})
end
figure(2);
for i = 1 : length(width_1)
    plot((peakTime_trial(i)-width_1(i)/0.32:peakTime_trial(i)+width_1(i)/0.32),LFP{i,:})
    hold on
end
