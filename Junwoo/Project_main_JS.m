...
[MAp_baseline,MAp_intense,MAp_poststimulus] = compartmentize_JS(filtered,baselinec,intensec,poststimulusc);
[MHp_filtered] = filtering_JS(MHp);
[MHp_baseline,MHp_intense,MHp_poststimulus] = compartmentize_JS(filtered,baselinec,intensec,poststimulusc);
...
subplot(3,1,3)
plot(t(samplerate*10.6+1:end-poststimulusc),MAp_afterstimulus,linespec{1,:},linespec2{1,:})
xlabel('Time (s)', fontsize{2,:}); ylabel('Amplitude (arb. Units)', fontsize{2,:})
...
subplot(3,1,3)
plot(t(samplerate*10.6+1:end-poststimulus),MHp_afterstimulus,linespec{1,:},linespec2{1,:})
xlabel('Time (s)', fontsize{2,:}); ylabel('Amplitude (arb. Units)', fontsize{2,:})
